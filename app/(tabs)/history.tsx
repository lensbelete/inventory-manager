  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-950" edges={['top', 'left', 'right']}>
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 32 }}>
        <Text className="pt-2 text-2xl font-semibold text-slate-100">Transaction history</Text>
        <Text className="mt-1 text-sm text-slate-400">
          Newest first. Use pagination to browse older entries.
        </Text>

        <View className="mt-4 flex-row items-center justify-between rounded-xl border border-slate-800 bg-slate-900/60 px-3 py-2">
        ...
        {pageItems.map((row) => (
          <View key={row.id} className="mt-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <Text className="mt-2 text-sm text-slate-300">
              ...
              {' · '}
              Qty after: <Text className="font-semibold text-slate-100">{row.quantityAfter}</Text>
            </Text>
            <Text className="mt-1 text-xs text-slate-500">By: {row.performedByLabel}</Text>
